...
public class GLAntiAliasConfigChooser implements GLSurfaceView.EGLConfigChooser {
    @Override
    public EGLConfig chooseConfig(EGL10 egl, EGLDisplay display) {
        ...
        for (int[] attribute : attributes) {
            ...
            if (configCounts[0] == 0) {
                Log.e(JoyUtils.TAG, "Unable to choose EGL config");
                GLUtils.checkError("GLAntiAliasConfigChooser.chooseConfig");
            } else {
                for (int i = 0; i < configCounts[0]; i++) {
                    int[] result = new int[1];
                    if (egl.eglGetConfigAttrib(display, configs[i], EGL10.EGL_SAMPLE_BUFFERS, result)) {
                        Log.d(JoyUtils.TAG, "EGL_SAMPLE_BUFFERS:" + result[0]);
                    } else {
                        GLUtils.checkError("GLAntiAliasConfigChooser.chooseConfig");
                    }
                    if (egl.eglGetConfigAttrib(display, configs[i], EGL10.EGL_SAMPLES, result)) {
                        Log.d(JoyUtils.TAG, "EGL_SAMPLES:" + result[0]);
                    } else {
                        GLUtils.checkError("GLAntiAliasConfigChooser.chooseConfig");
                    }
                }
